...
// 입력받은 x 값이 0과 10사이에 있는지 판단하는 로직   
    ...
    int isInside = xpt >= leftTopx && xpt <= rightBottomx && ypt >= leftTopy && ypt <= rightBottomy;
    ...
    // 1가지 조건이라도 아니라면 거짓 즉 false라서 isInside변수에0을 저장 
    // 결국 참이면 if (isInside == 1) 거짓이면 if (isInside == 0) 이렇게 식이 작성됨
    // 그래서 전자의 경우  printf("점(%d, %d)는 박스 안에 있습니다.\n", xpt, ypt); 가 출력되며
    // 후자의 경우 printf("점(%d, %d)는 박스 안에 없습니다.\n", xpt, ypt); 해당 내용이 출력됨
    // 그래서 isInside 함수를 사용해도 임의적으로 if (isInside == 0) 이렇게 설정한다면 
